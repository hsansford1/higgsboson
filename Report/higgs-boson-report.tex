...
\begin{equation}\label{eq:wlr_beta}
...
>>>>>>> f9ade5e2d8590f84654f49548eeb7a7300b20dc0
...
<<<<<<< HEAD
\subsection{Comments}
Elastic net (weighted) logistic regression, where coefficients are found by modifying equation \label{eq:wlr_beta} to include terms penalising their L1 and L2 norms
$$\hat{\bbeta}= \text{argmax}\left[ \left( \sum_{i\in D}-\log(1+\exp\{-w_i y_i\bbeta'\x \})\right)  + \lambda\left[ \frac{1-\alpha}{2}||\bbeta||^2_2 + \alpha||\bbeta||_1 \right] \right], 
$$
was also implemented (with standardised input data) for both the direct and two-stage approaches with hyperparameters $\alpha, \lambda$ selected by cross validation. In both cases $\lambda=0$ -- corresponding to the unpenalised model -- was found to be optimal. This is likely because the training dataset (250000 observations) was very large compared to the number of features (30); with higher-dimensional models, for example those including interaction terms, an elastic net regression may be beneficial.
=======
