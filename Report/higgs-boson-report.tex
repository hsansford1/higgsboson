...
As first attempt, a standard (i.e., $\theta=0.5$) logistic regression has been fitted on the train set. Cross validation suggested that if we tune the coefficients of the model in order to directly maximise AMS, then we cannot get a value of AMS bigger than $0.3$. This is a quite small value, as one can understand by results below. Many attempts get a value of AMS bigger than even on the test set. This suggests that tuning the coefficients of a standard logistic regression on the basis of AMS is not a good strategy.
