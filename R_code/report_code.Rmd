...
We then fit a WLR model using the new training set `Train`, before visualising how the AMS varies with threshold `theta`. This informs the users decision of how to set the parameters `theta_0` and `theta_1` in the function `threshold_CV` in the `higgsboson` package. We can see that the peak definitely occurs in the range 0 to 0.1.
```{r, warning=FALSE}
